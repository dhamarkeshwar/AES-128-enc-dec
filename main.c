...
static void sub_bytes(uint8_t *arr)
{
    for(int i=0;i<16;++i)
        *(arr+i)=sbox[*(arr+i)];
}
...
static void initial_encrypt(uint8_t *arr)
{
    for(int i=0;i<16;++i)
        *(arr+i)=*(arr+i)^round_key[0][i];
}

static void normal_encrypt_round(uint8_t *arr,int t)
{
    sub_bytes(arr);
    shift_rows(arr);
    mix_columns(arr);
    for(int i=0;i<16;++i)
        *(arr+i)=*(arr+i)^round_key[t][i];
}

static void last_encrypt_round(uint8_t *arr)
{
    sub_bytes(arr);
    shift_rows(arr);
    for(int i=0;i<16;++i)
...
static void encrypt(uint8_t *arr)
{
    initial_encrypt(arr);
    for(int j=1;j<10;++j)
        normal_encrypt_round(arr,j);
    last_encrypt_round(arr);
}

//# cbc encryption
static void CBC_encrypt(uint8_t arr[][16])
{
...
//# Test Case 1, copy the whole code below in main(), change text[] to check the program for different string values

//round_key_construction();
//char text[]="Hello! My name is world";
//
//srand(time(NULL));
//
